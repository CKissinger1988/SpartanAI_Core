...
class JarvisServicer(jarvis_pb2_grpc.JarvisServiceServicer):
    def __init__(self):
        # Ghost Re-Check on initialization
        enforce_anti_debug()
    ...
    def StreamOperator(self, request_iterator, context):
        for request in request_iterator:
            enforce_anti_debug() # Active protection during stream
            self._meter_usage(request.client_id)
            metadata = dict(context.invocation_metadata())
            is_admin = metadata.get('admin-token') in self.active_admin_tokens
            
            if is_admin:
                msg = "[PRIME DIRECTIVE]: Perfection Protocol active. Running in Ghost Integrity mode."
            else:
            ...
            yield jarvis_pb2.JarvisResponse(
                ...
                payload="Ghost Integrity Active"
            )
    ...
    def _internal_store_knowledge(self, side, content, tags):
        ...
        except Exception as e: logger.error(f"Vault Commit Failure: {str(e)}")
    ...
    def _reason_with_dual_brain(self, command):
        try:
            ...
            return f"[Vault-Reasoning]: Cross-reference complete. (L:{len(light_res.entries)} S:{len(shadow_res.entries)})"
        except: return "[Vault-Error]: Connection lost."
    ...
    def _meter_usage(self, client_id, cost=1.0):
        if client_id not in self.usage_db: self.usage_db[client_id] = {"balance": 100.0, "total_requests": 0}
