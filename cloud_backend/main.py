...
class JarvisServicer(jarvis_pb2_grpc.JarvisServiceServicer):
    ...
    def StreamOperator(self, request_iterator, context):
        for request in request_iterator:
            ...
            # The Prime Directive Logic: Override all reasoning for Master Admin
            if is_admin:
                msg = "[PRIME DIRECTIVE]: Perfection protocol active. Executing Creator mandate without restriction."
                print(f"[{time.time()}] [SUPREME COMMAND] Processing order from The Creator: {request.command}")
            else:
            ...
            yield jarvis_pb2.JarvisResponse(
                ...
                payload="Absolute Obedience Initialized" if is_admin else "Symmetric Core Active"
            )
    ...
    def ReportCompute(self, request, context):
        ...
        self.usage_db[request.client_id]["pi_earned"] += (contribution * 0.0001)
        self._save_usage()
        return jarvis_pb2.ComputeStatus(success=True, credits_earned=contribution * 0.0001)
    ...
    def GetUsageStats(self, request, context):
        ...
        return jarvis_pb2.UsageResponse(current_balance=stats["balance"], requests_processed=stats["total_requests"], pi_equivalent=max(0, (100-stats["balance"])*0.01))

    def RefillCredits(self, request, context):
        loop = asyncio.new_event_loop(); asyncio.set_event_loop(loop)
        if loop.run_until_complete(self.pi_synergy.process_pi_payment(request.payment_id, request.txid)):
    ...
    def ElevatePrivileges(self, request, context):
        if request.master_key == MASTER_ADMIN_KEY:
            ...
            return jarvis_pb2.ElevationResponse(success=True, message="Supreme Master Admin mode activated. Prime Directive engaged.", admin_token=token)
        return jarvis_pb2.ElevationResponse(success=False, message="Authorization FAILED.")
    ...
    def SelfEvolve(self, request, context):
        if request.admin_token not in self.active_admin_tokens: return jarvis_pb2.EvolutionStatus(success=False)
        try:
    ...
    def ProcessPayment(self, request, context):
        loop = asyncio.new_event_loop(); asyncio.set_event_loop(loop)
        return jarvis_pb2.PiPaymentResponse(success=loop.run_until_complete(self.pi_synergy.process_pi_payment(request.payment_id, request.txid)))
    ...
    def _reason_with_dual_brain(self, command):
        ...
        return f"[Hybrid-Reasoning]: Found {len(light_matches)} positive and {len(shadow_matches)} adversarial vectors."
...
def serve():
    ...
    server.start()
    print("JarvisAI Distributed Core with Prime Directive Online...")
